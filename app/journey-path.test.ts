...
describe('scroll route geometry', () => {
  it('connects actual milestone positions with vertically monotonic curves', () => {
    ...
    ).toBe('M 90 20 C 90 120, 160 120, 160 220 C 160 380, 90 380, 90 540');
  ...
  it('follows viewport height rather than assuming path length equals vertical distance', () => {
    const pointAt = (distance: number) => ({
      x: Math.sin(distance),
      y: 50 + (distance / 1000) ** 2 * 2000,
    });
    expect(distanceAtY(1000, 550, pointAt)).toBeCloseTo(500, 1);
    expect(distanceAtY(1000, 175, pointAt)).toBeCloseTo(250, 1);
  });
  it('provides clamped position and proximity for scroll choreography', () => {
    expect(scrollMotion(400, 800)).toEqual({ position: 0, proximity: 1 });
    expect(scrollMotion(-500, 800)).toEqual({ position: -1, proximity: 0 });
    expect(scrollMotion(1400, 800)).toEqual({ position: 1, proximity: 0 });
    expect(scrollMotion(0, 0)).toEqual({ position: 0, proximity: 1 });
  });
});
