...
The Academic Journey has a scroll-drawn route from education and internship milestones into a proposed future: UI/UX, AR/VR, AI-assisted experiences, then creative technology and leadership. These future chapters are labelled as aspirations. Cards drift, sharpen, and settle as they cross the reading line while markers, margin notes, and large chapter words move at different depths. A transparent Three.js scene follows the same progress: a rotating form travels through eight spatial milestone rings as the camera moves from cobalt foundations into the violet future. Each milestone also crossfades into its own visual environment—from classroom paper and school geometry through university interfaces, an industry network, UI wireframes, a spatial grid, AI nodes, and an open horizon. `app/JourneyPath.tsx` measures the actual card positions, including expanded project prompts, so the SVG path, 3D scene, and chapter backgrounds stay aligned on mobile and desktop. Reduced-motion preferences show the complete static route and skip WebGL. The academic page has its own brief, non-blocking entrance reveal.
