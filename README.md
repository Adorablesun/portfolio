...
The personal contact page is published at `personal/`. Its entrance types “Hi, I’m” and “Yu Yang” in sequence without changing their natural spacing or clipping letter descenders, then pulls the two halves of “Let’s connect” together with animated nodes and a light that keeps travelling along the finished connection. The OY identity card turns upright from a near-horizontal position and is built as a two-sided CSS 3D object with front, back, and edge surfaces. Visitors drag horizontally to rotate it continuously and release to settle on the nearest face; cursor movement adds live depth. Arrow keys provide the equivalent keyboard control. The front keeps its fractured reveal, while the reverse carries a second identity composition. Reduced-motion behavior is included. The contact area is an interactive communication console: WhatsApp, email, Instagram, and LinkedIn each have their own cursor-responsive signal animation while the full card remains an accessible link. Public contact details live in `app/portfolio.ts`; unavailable channels stay visibly disabled rather than linking to placeholder destinations.
