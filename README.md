...
The personal contact page is published at `personal/`. Its OY identity card uses a cursor-following glow and a fractured reveal on hover, with keyboard, touch-focus, and reduced-motion behavior. The contact area is an interactive communication console: WhatsApp, email, Instagram, and LinkedIn each have their own cursor-responsive signal animation while the full card remains an accessible link. Public contact details live in `app/portfolio.ts`; unavailable channels stay visibly disabled rather than linking to placeholder destinations.
